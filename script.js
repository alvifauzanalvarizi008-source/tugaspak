...
function setupLoginForm() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            if (email === 'admin@stta.ut.ac.id' && password === 'password123') {
            ...
            if (pengiriman) {
                ...
                resultDiv.innerHTML = `
                    <div class="tracking-details">
                        ...
                        <p><strong>Nomor Induk siswa:</strong> ${pengiriman.noDO}</p>
                        <p><strong>Nama siswa:</strong> ${pengiriman.namaMahasiswa}</p>
                        ...
                        <p><strong>Tanggal Pemberian Tugas:</strong> ${pengiriman.tanggalKirim}</p>
                        <p><strong>Nama Tugas:</strong> ${pengiriman.jenisPaket}</p>
                        <p><strong>Guru Pembimbing:</strong> ${pengiriman.gurupembimbing}</p>
                       
                    </div>
                `;
            } else {
                resultDiv.innerHTML = '<div class="alert alert-error">Nomor induk siswa tidak ditemukan</div>';
            }
        });
    }
}
...
function setupLaporan() {
    ...
    if (searchBtn) {
        searchBtn.addEventListener('click', function() {
            ...
            // Cari data pengiriman
            const pengiriman = dataLaporan.find(item => item.noInduk === noinduk);
            ...
            if (pengiriman) {
                ...
                resultDiv.innerHTML = `
                    <div class="laporan-details">
                        ...
                        <p><strong>Nomor Induk siswa:</strong> ${pengiriman.noInduk}</p>
                        <p><strong>Nama siswa:</strong> ${pengiriman.namaSiswa}</p>
                        
                        
                      
                        
                        
                        <p><strong>Alasan:</strong> ${pengiriman.Alasan}</p>
                        <p><strong>Mapel:</strong> ${pengiriman.mapel}</p>
                        <p><strong>Guru Pembimbing:</strong> ${pengiriman.gurupembimbing}</p>
...
// Fungsi untuk menampilkan data dummy di halaman laporan
function displayDummyDataInLaporan() {
    const tableBody = document.getElementById('dummyTableBodyLaporan');
    
    if (tableBody && dataLaporan) {
        tableBody.innerHTML = '';
        ...
        dataLaporan.forEach(item => {
            ...
            row.innerHTML = `
                <td><strong>${item.noInduk}</strong></td>
                <td>${item.namaSiswa}</td>
                <td>${item.Alasan}</td>
                <td>${item.mapel}</td>
                <td>${item.gurupembimbing}</td>
                <td class="action-buttons">
                    <button class="copy-btn-laporan" data-laporan="${item.noInduk}">Salin</button>
                    <button class="use-btn-laporan" data-laporan="${item.noInduk}">Gunakan</button>
                </td>
            `;
        ...
        // Setup event listener untuk tombol
        setupDummyTableButtonslaporan();
    }
}


// Setup tombol di tabel dummy tracking
    ...
    copyButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            navigator.clipboard.writeText(doNumber).then(() => {
                alert(`Nomor induk siswa ${doNumber} berhasil disalin!`);
            });
        });
    });
...
// Setup tombol di tabel dummy laporan
function setupDummyTableButtonslaporan() {
    // Tombol "Salin"
    const copyButtonslaporan = document.querySelectorAll('.copy-btn-laporan');
    copyButtonslaporan.forEach(buttonlaporan => {
        buttonlaporan.addEventListener('click', function() {
            const Laporansiswa = this.getAttribute('data-laporan');
            navigator.clipboard.writeText(Laporansiswa).then(() => {
                alert(`nomor induk siswa ${Laporansiswa} berhasil disalin!`);
            });
        });
    });
    
    // Tombol "Gunakan"
    const useButtonslaporan = document.querySelectorAll('.use-btn-laporan');
    useButtonslaporan.forEach(buttonlaporan => {
        buttonlaporan.addEventListener('click', function() {
            const noInduk = this.getAttribute('data-laporan');
            const Inputlaporan = document.getElementById('noInduk');
            
            if (Inputlaporan) {
                Inputlaporan.value = noInduk;
                Inputlaporan.focus();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Tampilkan data dummy di halaman laporan
     if (window.location.pathname.includes('laporan.html')) {
        displayDummyDataInLaporan();
    }
});